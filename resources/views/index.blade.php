            ...
            <tbody>
                @if($isSpecialUser)
                    <div class="alert alert-info">
                        Siz maxsus foydalanuvchisiz (Special User)!
                    </div>
                @endif
                @forelse ($permissions as $permission)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-4 py-2 border-b">{{ \Carbon\Carbon::parse($permission->from_time)->format('Y-m-d H:i') }}
                        </td>
                        <td class="px-4 py-2 border-b">{{ \Carbon\Carbon::parse($permission->to_time)->format('Y-m-d H:i') }}
                        </td>
